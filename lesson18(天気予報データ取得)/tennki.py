...
from get_stateless_access import get_stateless_access_token
from bs4 import BeautifulSoup

url =  "https://api.aoikujira.com/index.php?tenki&show#h39c8f4bf"

#BeautifulSoupにそのままURLを指定するとエラーが起こるため、requestsを用いてドキュメントを取得する

def get_document_from_web(url):
  response = requests.get(url)
  soup = BeautifulSoup(response.content, 'html.parser')
#  tenki = soup.find("tbody")


# メッセージファイル名
# infile_path = os.path.abspath(os.path.dirname(__file__)) + './tennkimessages.json'

# チャネルIDとチャネルシークレット
# LINE Official Account Managerの Messaging API画面で確認する
# 送信者に相当する
client_id = "2007620309"
client_secret = "d7c0bf266b7e9a513b7669e8b3963084"
